...
function doc(int $item_id , string $item_name , string $item_page){
    ...
    <?php if ($user->data['user_id'] != ANONYMOUS) { ?>
        ...
        <input type="hidden" name="category" value="<?= $item_page ?>">
            <input type="hidden" name="item_id" value="<?= $item_id ?>">
            <textarea name="comment" required placeholder="Add a Comment..." style="width:100%; height:80px;"></textarea>
            <button type="submit" style="background-color:#6d28d9; color:white; padding:10px 15px; border:none; border-radius:5px; cursor:pointer;">Send</button>
        </form>
    <?php } else { ?>
        ...
        foreach ($comments as $comment) {
            ...
            if ($comment['user_id'] == $user->data['user_id'] || $user->data['user_id'] == 2) {
                echo "<form action='delete_comment.php' method='POST' style='display:inline;'>
                        ...
                        <button type='submit' style='background-color:#dc2626; color:white; padding:5px 10px; border:none; border-radius:5px; cursor:pointer;'>Delete</button>
                      </form>";
            }
